...
export default defineComponent({
  ...
  async created() {
  try {
    if(flag_k!=1){const response = await axios.get("https://swen90016.herokuapp.com/request/item");
  ...
  methods: {
   redirectToAdditionalDetail(record) {
     ...
     this.$router.push({ name: "ItemDetail", params: {  userID: localStorage.getItem("userID"), itemID: record._id } });    },
